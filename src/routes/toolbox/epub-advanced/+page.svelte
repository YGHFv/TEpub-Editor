<script lang="ts">
  ...
  const CONFIG: Record<WebEpubAdvancedAction, { title: string; icon: string; detail: string; multiple: boolean }> = {
    ...
    "epub-version": { title: "EPUB 版本转换", icon: "2/3", detail: "在 EPUB 2.0 与 3.0 间转换，并自动补齐 NCX 或 NAV 导航。", multiple: false },
    "epub-chinese": { title: "EPUB 简繁转换", icon: "繁简", detail: "转换正文、目录和 OPF 元数据中的中文，不改动标签与样式。", multiple: false },
    ...
    "epub-footnote": { title: "批注与脚注转换", icon: "NOTE", detail: "增强标准 EPUB3 脚注的弹窗数据，或将带文本属性的弹窗批注转为标准脚注。", multiple: false },
    "image-compress": { title: "EPUB 图片压缩", icon: "IMG", detail: "浏览器本地重编码并按需缩放 JPEG、PNG 和 WebP，较大结果不会写回。", multiple: false },
    "image-watermark": { title: "图片隐形水印", icon: "WM", detail: "把带校验的 UTF-8 文本写入图片 RGB 最低位，或扫描读取 TEpub 水印。", multiple: false },
    "epub-merge": { title: "合并 EPUB", icon: "MERGE", detail: "保留各书资源目录和相对引用，重新生成合集 manifest、spine、NAV 与 NCX。", multiple: true },
    "epub-split": { title: "按章节拆分 EPUB", icon: "SPLIT", detail: "预览阅读顺序条目，按指定章节数生成多本结构完整的 EPUB。", multiple: false },
  };
  ...
  async function run() {
    ...
    try {
      result = await processWebEpubAdvanced(files, action, options);
...
<div class="advanced-page">
  <header class="topbar">
    <div class="heading">
      ...
      <div><span>SHARED EPUB TOOL</span><h1>{config.title}</h1></div>
    </div>
    <div class="head-actions">
      {#if outputUrls.length > 1}<button type="button" on:click={downloadAll}>下载全部</button>{/if}
      <button class="primary" type="button" disabled={busy} on:click={() => fileInput?.click()}>{files.length ? "重新选择" : config.multiple ? "选择多个 EPUB" : "选择 EPUB"}</button>
    </div>
  </header>
  ...
  <main class="content">
    ...
    {:else}
      <section class="selection">
        ...
        <button class="primary run" type="button" disabled={busy || (action === "epub-merge" && files.length < 2)} on:click={run}>{busy ? "处理中..." : options.watermarkMode === "inspect" && action === "image-watermark" ? "扫描水印" : "开始处理"}</button>
      </section>
    {/if}
