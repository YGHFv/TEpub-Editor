...
### 2026-07-18 v1.0.1 EPUB 样式库与在线图片下载

- 将“EPUB 样式预览”扩展为 EPUB 样式库，新增内置插图样式、注释弹图样式和转场样式。
- 工具箱新增“EPUB 在线图片下载”，识别真实与转义的远程 `img` 标签，下载图片后写入 EPUB 资源、OPF manifest 和章节引用。
- 在线图片支持失败后慢速自动重试、失败项手动重试、样式预览和独立下载日志弹窗。
- 网页版按“直接 Fetch、匿名图片 Canvas 转换、同源服务端代理”依次尝试，桌面端使用受限远程图片下载命令。
